...
test('basic table renders with data', async ({ page }) => {
  ...
  await expect(page.locator('h2')).toHaveText('Basic Table')
...
test('navigation between demo pages', async ({ page }) => {
  ...
  await page.getByText('Editing & Validation').click()
  await expect(page.locator('h2')).toHaveText('Editing & Validation')
  await expect(page.getByText('Errors:')).toBeVisible()

  // Switch to Virtual Scroll demo
  await page.getByText('Virtual Scroll (10k rows)').click()
  await expect(page.locator('h2')).toHaveText('Virtual Scroll (10,000 rows)')
})
...
test('validation demo shows errors', async ({ page }) => {
  ...
  await expect(page.getByText(/Errors: \d+/)).toBeVisible()
