...
├── packages/
│   └── core/                   # ライブラリ本体 (@heynow/react-spread-sheet-table)
│       ├── package.json
...
- `packages/core`をworkspace依存として参照（`"@heynow/react-spread-sheet-table": "workspace:*"`）
...
### apps/playground
- **dependencies**: react, react-dom, `@heynow/react-spread-sheet-table` (workspace:*)
